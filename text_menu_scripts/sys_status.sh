#!/bin/sh
# !DESC-EN!10!sys_status!System status; memory/disk usage and system load
...
# $Id: sys_status.sh,v 1.1 2009-07-23 08:30:56 brian Exp $
...
# FIXME 
# change this so that the below output is collected into individual shell
# variables, and then combine them when displaying the dialog, not before
SYS_MSG=$(echo 'System uptime\n'; uptime)
SYS_MSG=$(echo "${SYS_MSG}\nMemory Usage\n"; free)
SYS_MSG=$(echo "${SYS_MSG}\nDisk Usage\n";df -h; echo "\n")
### SCRIPT MAIN LOOP ###
dialog --backtitle "Propaganda: System Status" \
--title "System Status" --begin 2 2 --msgbox "$SYS_MSG" 21 75 \
    2> /tmp/dialog.exit
