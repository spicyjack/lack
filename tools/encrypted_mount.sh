...
#
# Mount loop-aes encrypted filesystems
...
if [ $QUIET -eq 0 ]; then
    # run the script
    echo
    echo "Disk key ID:      '${KEY_ID}'"
    echo "Disk key size:    '${KEY_SIZE}'"
...
/sbin/losetup -G /home/brian/keyhosts/${KEY_ID} \
    -K /home/brian/keyhosts/${KEY_ID}/disk_key.gpg -p 0 \
    -e AES${KEY_SIZE} /dev/loop0 ${DEVICE} 2>&1)
